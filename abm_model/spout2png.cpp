...
int main(int argc, char **argv)
{
	...
	while(true) {
		...
		double def = 0.0, conf = 0.0;
		bool have_line = rt.read_line();
		if(have_line) {
			if(!rt.read(year0, id, pop)) break;
			if(!rt.read(raiders)) break;
			if(!rt.read(def, conf)) break;
		}
		...
		if(year % year_step == 0) {
			if(use_cellnet) {
				...
				double val = 0.0;
				poly_output::color_scale_t cs;
				if(pop) {
					val = std::min((double)pop, pars.max_pop) / pars.max_pop;
					if(!pars.raider_pop && raiders) val = 1.0;
					cs = raiders ? poly_output::color_scale_t::RED : poly_output::color_scale_t::BLUE;
				}
				else {
					val = conf;
					cs = poly_output::color_scale_t::YELLOW;
				}
				pout.draw_cell(id, val, cs);
			}
		}
	}
