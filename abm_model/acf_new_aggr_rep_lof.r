# acf_new_aggr_rep_lof.r -- parallel analysis for processing new aggregated
#  simulation results; version for repeated simulation realizations
#  alternative methodology, no detrending, but ignoring a warm-up period

library(foreach)
library(doParallel)


## !! TODO: potentially set the correct working directory !!
# setwd('/mnt/smallext/kondor')
## !! TODO: set the maximum number of CPU cores to be used !!
ncores = 50
...
do_one_acf = function(res1, tmin1, tmax1) {
  sum_res1 = list()
  ...
  for(i in 1:nrow(aggr_types)) {
    ...
    for(ID1 in unique(tmp1$ID)) {
      ...
      # alternate approach: detrending only with the mean
      N = mean(tmp2$pop)
      tmp2$pred = N
...
# alternate approach: instead of detrending, just skip the first 5000 years
tmin1 = 5000
tmax1 = 10000
